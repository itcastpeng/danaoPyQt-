...
class PC_SOUGOU_URL_PC():
    ...
    def __init__(self, keyword, domain):
        ...
        self.pc_sougouurl = """https://www.sogou.com/web?query={}""".format(keyword)
    ...
    def get_keywords(self):
        ...
        # shoulu = '0'
        ret = requests.get(self.pc_sougouurl, headers=headers)
        soup = BeautifulSoup(ret.text, 'lxml')
        results_tag_count = soup.find('div', class_='results')
        data_list_count = []
        results_tags = results_tag_count.find_all('div', class_='vrwrap')
        rb_tags = results_tag_count.find_all('div', class_='rb')
        for results_tag in results_tags:
            data_list_count.append(results_tag)
        for rb_tag in rb_tags:
            data_list_count.append(rb_tag)
        for data in data_list_count:
            paiming = ''                                        # 排名
            if data.find('div', class_='fb'):
                paiming_gewei = data.find('a').attrs.get('id')[-1]
                if paiming_gewei == '0':
                    paiming_shiwei = data.find('a').attrs.get('id')[-2]
                    if paiming_shiwei == '_':
                        paiming = paiming_gewei
                    else:
                        paiming = paiming_shiwei + paiming_gewei
                else:
                    paiming = paiming_gewei
                yuming_init = data.find('div', class_='fb').find('cite').get_text().split('...')
                yuming = ''                                     # 判断域名
                if ' - ' in yuming_init[0]:
                    yuming = yuming_init[0].split(' - ')[1]
                else:
                    yuming = yuming_init[0]
                kuaizhao_time = ''
                if '翻译此页' in yuming_init[1]:
                    kuaizhao_time_data = yuming_init[1][:-4]
                else:
                    kuaizhao_time_data = yuming_init[1]
                kuaizhao_time = kuaizhao_time_data.replace(kuaizhao_time_data[:3], '')
                # if yuming in self.domain:
                if 'http' in data.find('a', target='_blank').attrs['href']:
                    qingqiu_url = data.find('a', target='_blank').attrs['href']
                else:
                    qingqiu_url = 'https://www.sogou.com' + data.find('a', target='_blank').attrs['href']
                sleep(1)
                ret_two = requests.get(qingqiu_url, headers=headers)
                sleep(1)
                print(ret_two.url)
...
#         domain = 'http' + data.strip().split('http')[1]
        ...
        # PC_SOUGOU_URL_PC(keyword, domain)

keyword = '百姓呼声:“私人健康顾问服务”正式在岳阳世纪医院落实运行	'
domain = 'http://www.sohu.com/a/208330800_544906'
PC_SOUGOU_URL_PC(keyword, domain)
